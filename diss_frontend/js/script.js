...
Note: The basic UI features in this file (like updating the chat window and saving chat history)
were written with the assistance from Google Gemini.
...
//This variable is used to keep track of what chat is currently open.
...
function loadSavedTheme() {
    ...
    // 2. Apply dark mode if that is what the user has saved.
    if (saved_theme === 'dark') {
        document.body.classList.add('dark-mode');
    }
}

// Load saved text preference based initialisation.
...
// Load saved text preference on initialisation.
...
function toggleSidebar() {
    ...
    // Toggle menu icon visibility based on sidebar state.
...
// Fetch and render the chat history from the local storage.
function loadOldGuidanceSessions() {
    ...
    // Get the saved chats or create a blank array if it is the first time the user is using the system.
    ...
    */
    saved_chats.forEach(chat => {
        ...
        delete_button.onclick = (event) => {
            // To prevent loading the chat when deleting.
...
// Function used to delete the specific chat.
function deleteSessionRecord(id) {
    if (confirm("Are you sure you want to delete this chat?")) {
        ...
        // Reset the view if currently active chat is deleted.
...
// Append new messages to local storage session.
function saveChatSession(patient_query, system_guidance, medical_safety_flags) {
    ...
    // If the current chat ID is unavailable it means that it is a new conversation, so create the structure for it.
    if (current_chat_index === -1) {
        const new_chat = {
            id: active_conversation_session_id,
            // Get the first 30 characters of the user's message to use it as a summary of the chat.
...
// Load a specific past chat session into the UI.
function loadSpecificSession(id) {
    ...
    if (chat_to_load) {
        ...
        chat_to_load.messages.forEach(msg => {
            // Map backend roles to UI CSS classes.
...
// This function is used to render the chat bubble elements.
function displayChatBubble(text, message_source) {
    ...
    // Uses a third party marked.js library to translate the LLMs markdown syntax into HTML bullet points and bold text.
...
// Reset the state of the UI for a new chat.
...
async function submitFollowUpQuery() {
    ...
    input_field.value = "";
    // Passes the follow up query to the server.
